...
api_key = os.getenv("GROQ_API_KEY_MOMENTUM") or os.getenv("GROQ_API_KEY", "")
llm = LLM(
    model=os.getenv("MODEL_MOMENTUM") or os.getenv("GROQ_MODEL", "groq/llama-3.3-70b-versatile"),
    api_key=api_key,
    temperature=0.3
)
...
def build_defense_task(agent: Agent, original_proposal: BandMessage, challenges: list[BandMessage]) -> Task:
    ...
    return Task(
        description=(
            f"You are defending your original Momentum proposal against challenges from review agents.\n\n"
...
def run_defense_agent(original_proposal: BandMessage, challenges: list[BandMessage]) -> BandMessage:
    ...
    print("[momentum_agent] Starting defense crew run...")
    result = crew.kickoff()
    
    output_text = str(result)
    band_msg = defense_to_band_message(output_text)
    
    print(f"[momentum_agent] Defense ready → {band_msg.content}")
    return band_msg
